...
import os
from evaluator import retrieval_evaluate
...
def create_retrieval_dataloader(ds, 
    ...
    # Filter by context_length
    filtered_qrels = [qrel for qrel in qrels if qrel.get("context_length", 0) == context_length]
    
    # Get the set of valid query and document IDs from filtered qrels
    valid_qids = set(qrel["qid"] for qrel in filtered_qrels)
    valid_doc_ids = set(qrel["doc_id"] for qrel in filtered_qrels)
    
    # Filter queries and corpus based on valid IDs
    queries = [q for q in queries if q["qid"] in valid_qids]
    corpus = [d for d in corpus if d["doc_id"] in valid_doc_ids]
    qrels = filtered_qrels
    print("Queries number:", len(filtered_qrels))
    print("Documents number:", len(corpus))
    print("Qrels number:", len(qrels))
    ...
    if sentence_process_function is not None:
        query_dataset = Dataset.from_list([{"texts": sentence_process_function(q["text"]), "qid": q["qid"]} for q in queries])
    else:
    ...
    if sentence_process_function is not None:
        document_dataset = Dataset.from_list([{"texts": sentence_process_function(d["text"]), "doc_id": d["doc_id"]} for d in corpus])
    else:
...
class RetrievalWrapper(nn.Module):
    ...
    def _do_encode_embeddings(self,
        ...
        for batch in tqdm.tqdm(query_dataloader, desc='encoding', mininterval=10):
            ...
            raw_outputs = self.model(output_hidden_states=True, **batch)
...
def main():
    ...
    parser.add_argument("--context_length", type=int, default=None,
                        help="The context length of datasets")
    ...
    # Load data task
    data = yaml_config['data']['long_context_synthetic_task']
    ...
    cfg.data.context_length = args.context_length or data.get('context_length', 256)

    hyper_parameters = textwrap.dedent(f"""
        Configuration:
        -------------
        Data contect length     : {cfg.data.context_length}
    ...
    if cfg.architectures.use_which_plan != 'vanilla':
        ...
        #     embedding_layer.weight[begin_placeholder_token_id] = torch.randn(num_dim, device=device)
        # embedding_layer.weight.requires_grad_(False)
    ...
    results_file = f"results/{name}/synthetic_retrieval_results.txt"  # File to save results
    ...
    for task in tasks:
        ...
        with open(results_file, "a+") as f:
            f.write(f"Task: {task}\n")
            f.write(f"Context length: {cfg.data.context_length}\n")
